...
/**
 * true, если приложение реально открыто внутри Telegram И клиент достаточно новый
 * для поддержки CloudStorage (метод появился в Bot API 6.9). Старые версии клиента
 * (например 6.0) имеют объект CloudStorage в window, но вызов его методов бросает
 * "WebAppMethodUnsupported" -- поэтому одной проверки наличия объекта недостаточно.
 */
...
export const cloudStorage = {
  setItem(key: string, value: string): Promise<void> {
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.setItem(key, value);
  ...
  getItem(key: string): Promise<string | null> {
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.getItem(key);
    return new Promise((resolve, reject) => {
  ...
  getItems(keys: string[]): Promise<Record<string, string>> {
    ...
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.getItems(keys);
    return new Promise((resolve, reject) => {
  ...
  removeItem(key: string): Promise<void> {
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.removeItem(key);
    return new Promise((resolve, reject) => {
  ...
  removeItems(keys: string[]): Promise<void> {
    if (keys.length === 0) return Promise.resolve();
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.removeItems(keys);
    return new Promise((resolve, reject) => {
  ...
  getKeys(): Promise<string[]> {
    const tg = getWebApp();
    if (!isInsideTelegram() || !tg) return localFallback.getKeys();
    return new Promise((resolve, reject) => {
